...
public class AverageResponseTime extends Analytics
{
	protected void printCsv (final String from, final String to)
		throws ParseException
	{
		final String tz = "UTC";
		log ("Scan for response times between " + from + " and " + to +
			" (" + tz + ")");
		...
		System.out.println ("\"Date\",\"Sum Response Times\",\"Sum Calls\"");
		...
		for (int n = 0; n < results.size (); n++)
		{
			...
			System.out.println ("\"" + convert (timestamp) + "\"," +
				a.getJsonNumber (1).doubleValue () + "," + a.getInt (2));
	...
	public static void main (final String argv[])
	{
		try
		{
            if (argv.length < 2)
            {
                System.err.println ("Usage: psd2.AverageResponseTime");
			...
			log ("from: \"" + from + "\"");
			log ("to: \"" + to + "\"");
			AverageResponseTime art = new AverageResponseTime ();
			art.init ();
			art.printCsv (from, to);
			log ("More data is " + art.moreData ());
		}
		catch (Exception x)
		{
			x.printStackTrace ();
